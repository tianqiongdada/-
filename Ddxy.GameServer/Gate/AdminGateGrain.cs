...
namespace Ddxy.GameServer.Gate
{
    ...
    public class AdminGateGrain : Grain, IAdminGateGrain
    {
        ...
        {
            // 记录操作日志
            LogOp(ip, "signin|start", Json.Serialize(new { username, password }));
            // 过滤注入
            if (CheckPayload(username) || CheckPayload(password))
            {
                // 记录操作日志
                LogOp(ip, "signin|fail_sql_check", Json.Serialize(new { username, password }));
            ...
            await DbService.Sql.Update<AdminEntity>()
                ...
                .ExecuteAffrowsAsync();
            // 时间戳+[1000,9999]作为本地安全检查值
            var userData = TimeUtil.TimeStamp.ToString() + "|" + _random.Next(1000, 10000).ToString();
            // 构建token
            ...
            await RedisService.SetAdminSec(entity.Id, userData, (uint)_jwtOptions.ExpiresMinutes);
            await RedisService.SetAdminInfo(entity.Id, entity.Category, (uint)_jwtOptions.ExpiresMinutes);
            await RedisService.SetAdminAgencyInfo(entity.Id, entity.Agency, (uint)_jwtOptions.ExpiresMinutes);
            // 记录操作日志
            LogOp(ip, "signin|okay", Json.Serialize(new { username, password }), entity.Id);
        ...
        /// 处理登录后的所有操作
        /// </summary>
        public async Task<Immutable<byte[]>> Handle(string sec, uint opUid, string ip, string method, Immutable<byte[]> payload)
        {
            // 记录操作日志
            LogOp(ip, method + "|start", payload, opUid);
            // 过滤注入
            if (CheckPayload(payload))
            {
                // 记录操作日志
                LogOp(ip, method + "|fail_sql_check", payload, opUid);
                return JsonResp.BadOperation().Serialize();
            }
            ...
            // 检查本地安全值
            var secCheck = await RedisService.GetAdminSec(opUid);
            if (!secCheck.Equals(sec))
            {
                // 记录操作日志
                LogOp(ip, method + "|fail_local_sec_check", payload, opUid);
                return JsonResp.Unauthorized().Serialize();
            }
            ...
            switch (method)
            {
                ...
                case "list_admin":
                {
                    ...
                    resp = await ListAdmin(opUid, category, req);
                }
                    break;
                case "add_admin":
                {
                    var req = ParsePayload<AddAdminReq>(payload);
                    resp = await AddAdmin(opUid, category, req);
                }
                    break;
                case "del_admin":
                {
                    var req = ParsePayload<DelAdminReq>(payload);
                    resp = await DelAdmin(opUid, category, req.Id.GetValueOrDefault());
                }
                    break;
                case "edit_admin":
                {
                    var req = ParsePayload<EditAdminReq>(payload);
                    resp = await EditAdmin(opUid, category,req);
                }
                    break;
                case "froze_admin":
                {
                    var req = ParsePayload<FrozeAdminReq>(payload);
                    resp = await FrozeAdmin(opUid, category, req);
                }
                    break;
                case "list_agency":
                {
                ...
                case "set_notice":
                {
                    ...
                    resp = await SetNotice(opUid, category, req.Text);
                }
                    break;
                case "get_pay_enable":
                {
                ...
                case "set_pay_enable":
                {
                    ...
                    resp = await SetPayEnable(opUid, category, req.Enable);
                }
                    break;
                case "get_pay_rate":
                {
                ...
                case "set_pay_rate":
                {
                    ...
                    resp = await SetPayRate(opUid, category, req.Rate.GetValueOrDefault(), req.BindRate.GetValueOrDefault());
                }
                    break;
                case "list_mail":
                {
                    var req = ParsePayload<ListMailReq>(payload);
                    resp = await ListMail(opUid, category, req);
                }
                    break;
                case "add_mail":
                {
                    var req = ParsePayload<AddMailReq>(payload);
                    resp = await AddMail(opUid, category, req);
                }
                    break;
                case "del_mail":
                {
                    var req = ParsePayload<DelMailReq>(payload);
                    resp = await DelMail(opUid, category, req);
                }
                    break;
                ...
                case "set_res_version":
                {
                    ...
                    resp = await SetResVerion(opUid, category, req);
                }
                    break;
                case "reload_config":
                {
                    resp = await ReloadConfig(opUid, category);
                }
                    break;
                case "list_server":
                {
                    var req = ParsePayload<ListPageReq>(payload);
                    resp = await ListServer(opUid, category, req);
                }
                    break;
                case "list_server1":
                {
                    resp = await ListServer1(opUid, category);
                }
                    break;
                case "add_server":
                {
                    var req = ParsePayload<AddServerReq>(payload);
                    resp = await AddServer(opUid, category, req);
                }
                    break;
                case "edit_server":
                {
                    var req = ParsePayload<EditServerReq>(payload);
                    resp = await EditServer(opUid, category, req);
                }
                    break;
                case "change_server_status":
                {
                    var req = ParsePayload<ChangeServerStatusReq>(payload);
                    resp = await ChangeServerStatus(opUid, category, req);
                }
                    break;
                case "start_server":
                {
                    var req = ParsePayload<StartServerReq>(payload);
                    resp = await StartServer(opUid, category, req.Id);
                }
                    break;
                case "stop_server":
                {
                    var req = ParsePayload<StartServerReq>(payload);
                    resp = await StopServer(opUid, category, req.Id);
                }
                    break;
                case "open_activity":
                {
                    var req = ParsePayload<OpenActivityReq>(payload);
                    resp = await OpenActivity(opUid, category, req);
                }
                    break;
                case "query_combine_server":
                {
                    resp = await QueryCombineServer(opUid, category);
                }
                    break;
                case "combine_server":
                {
                    var req = ParsePayload<CombineServerReq>(payload);
                    resp = await CombineServer(opUid, category, req);
                }
                    break;
                case "list_user":
                {
                ...
                case "froze_device":
                {
                    var req = ParsePayload<FrozeDeviceReq>(payload);
                    resp = await FrozeDevice(opUid, category, req);
                }
                    break;
                case "froze_user":
                {
                ...
                case "change_role_star":
                {
                    ...
                    resp = await ChangeRoleStar(opUid, category, req);
                }
                    break;
                case "add_role_skillexp":
                {
                    var req = ParsePayload<GetRoleDetailReq>(payload);
                    resp = await AddRoleSkillExp(opUid, category, req);
                }
                    break;
                case "add_role_equip":
                {
                    var req = ParsePayload<AddRoleEquipReq>(payload);
                    resp = await AddRoleEquip(opUid, category, req);
                }
                    break;
                case "add_role_ornament":
                {
                    var req = ParsePayload<AddRoleOrnamentReq>(payload);
                    resp = await AddRoleOrnament(opUid, category, req);
                }
                    break;
                case "add_role_wing":
                {
                    var req = ParsePayload<AddRoleWingReq>(payload);
                    resp = await AddRoleWing(opUid, category, req);
                }
                    break;
                case "add_role_title":
                {
                    var req = ParsePayload<AddRoleTitleReq>(payload);
                    resp = await AddRoleTitle(opUid, category, req);
                }
                    break;
                case "del_role_title":
                {
                    var req = ParsePayload<DelRoleTitleReq>(payload);
                    resp = await DelRoleTitle(opUid, category, req);
                }
                    break;
                case "del_role_shane":
                {
                    var req = ParsePayload<DelRoleShaneReq>(payload);
                    resp = await DelRoleShane(opUid, category, req);
                }
                    break;
                case "set_role_type":
                {
                    var req = ParsePayload<SetRoleTypeReq>(payload);
                    resp = await SetRoleType(opUid, category, req);
                }
                    break;
                case "set_role_flag":
                {
                ...
                case "recharge_role":
                {
                    ...
                    resp = await RechargeRole(opUid, category, req, false);
                }
                    break;
                case "bind_recharge_role":
                {
                    // 要求管理员权限
                    if (category >= AdminCategory.Agency)
                    {
                        resp = JsonResp.Error(ErrCode.NoPermission);
                    }
                    else
                    {
                        var req = ParsePayload<RechargeRoleReq>(payload);
                        resp = await RechargeRole(opUid, category, req, true);
                    }
                }
                    break;
                case "list_recharge":
                ...
                case "list_rank_level":
                {
                    ...
                    resp = await ListRankLevel(opUid, category, req);
                }
                    break;
                case "list_rank_jade":
                {
                    var req = ParsePayload<ListRankReq>(payload);
                    resp = await ListRankJade(opUid, category, req);
                }
                    break;
                case "list_rank_pay":
                {
                    var req = ParsePayload<ListRankReq>(payload);
                    resp = await ListRankPay(opUid, category, req);
                }
                    break;
                case "list_rank_sldh":
                {
                    var req = ParsePayload<ListRankReq>(payload);
                    resp = await ListRankSldh(opUid, category, req);
                }
                    break;
                case "list_rank_sect":
                {
                    var req = ParsePayload<ListRankReq>(payload);
                    resp = await ListRankSect(opUid, category, req);
                }
                    break;
                case "dismiss_sect":
                {
                    var req = ParsePayload<DissmissSectReq>(payload);
                    resp = await DismissSect(opUid, category, req);
                }
                    break;
                case "reload_sects":
                {
                    var req = ParsePayload<ReloadSectsReq>(payload);
                    resp = await ReloadSects(opUid, category, req);
                }
                    break;
                case "list_rank_single_pk":
                {
                    var req = ParsePayload<ListRankReq>(payload);
                    resp = await ListRankSinglePk(opUid, category, req);
                }
                    break;
                case "list_log_op":
                {
                    var req = ParsePayload<ListOpLogReq>(payload);
                    resp = await ListOpLog(opUid, category, req);
                }
                    break;
                case "list_log_bug":
                {
                    var req = ParsePayload<ListBugLogReq>(payload);
                    resp = await ListBugLog(opUid, category, req);
                }
                    break;
                case "del_log_bug":
                {
                ...
                case "send_palace_notice":
                {
                    ...
                    resp = await SendPalaceNotice(opUid, category, req.Id, req.Msg, req.Times);
                }
                    break;
                case "send_role_gift":
                {
                    var req = ParsePayload<SendRoleGiftReq>(payload);
                    resp = await SendRoleGift(opUid, category, req.Id);
                }
                    break;
                case "send_set_limit_charge_rank":
                {
                    var req = ParsePayload<SendSetLimitChargeRankReq>(payload);
                    resp = await SendSetLimitChargeRank(opUid, category, req.Server, req.Start, req.End, req.Cleanup);
                }
                    break;
                case "send_get_limit_charge_rank":
                {
                    var req = ParsePayload<SendGetLimitChargeRankReq>(payload);
                    resp = await SendGetLimitChargeRank(opUid, category, req.Server);
                }
                    break;
                case "list_limit_charge_rank":
                {
                    var req = ParsePayload<ListLimitRankReq>(payload);
                    resp = await ListLimitRank(opUid, category, req);
                }
                    break;
                case "list_chat_log":
                {
                ...
                case "gen_cd_key":
                {
                    var req = ParsePayload<GenCDKeyReq>(payload);
                    resp = await DelGenCDKey(opUid, category, req);
                }
                    break;
            ...
            {
                // 记录操作日志
                LogOp(ip, method + "|fail_bad_method", payload, opUid);
            }
            else
            {
                // 记录操作日志
                LogOp(ip, method + "|okay", payload, opUid);
            }
        ...
        private async Task<JsonResp> ListAdmin(uint opUid, AdminCategory category, ListPageReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> AddAdmin(uint opUid, AdminCategory category, AddAdminReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var repo = DbService.Sql.GetRepository<AdminEntity>();
        ...
        private async Task<JsonResp> DelAdmin(uint opUid, AdminCategory category, uint id)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> EditAdmin(uint opUid, AdminCategory category, EditAdminReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            string safePwd = null;
        ...
        private async Task<JsonResp> FrozeAdmin(uint opUid, AdminCategory category, FrozeAdminReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListAgency(uint opUid, AdminCategory opCategory, ListAgencyReq req)
        {
            ...
            else
            {
                ...
                selector = DbService.Sql.Queryable<AdminEntity>()
                    .Where(it => it.ParentId == viewId)
                    .AsTreeCte()
                    .From<Admin1Entity>((_, b) => _);
            }
        ...
        private async Task<JsonResp> FrozeAgency(uint opUid, AdminCategory opCategory, FrozeAdminReq req)
        {
            ...
            var status = req.Status == 0 ? AdminStatus.Normal : AdminStatus.Frozen;
            // 只有管理员及以上才有解封权限
            if (status == AdminStatus.Normal && opCategory >= AdminCategory.Agency)
            {
                return JsonResp.NoPermission();
            }
            var er = await DbService.Sql.Update<AdminEntity>()
        ...
        private async Task<JsonResp> SetNotice(uint opUid, AdminCategory category, string notice)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            notice ??= "";
        ...
        private async Task<JsonResp> SetPayEnable(uint opUid, AdminCategory category, bool enable)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> SetPayRate(uint opUid, AdminCategory category, uint rate, uint bindRate)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListMail(uint opUid, AdminCategory category, ListMailReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
            ...
            var rows = await DbService.Sql.Select<MailEntity, ServerEntity, AdminEntity>()
                ...
                .WhereIf(searchId == 0 && req.Picked.HasValue && !req.Picked.Value, (a, b, c) => a.PickedTime == 0)
                .WhereIf(searchId == 0 && req.Removed.HasValue && req.Removed.Value, (a, b, c) => a.DeleteTime > 0)
                .WhereIf(searchId == 0 && req.Removed.HasValue && !req.Removed.Value, (a, b, c) => a.DeleteTime == 0)
        ...
        private async Task<JsonResp> AddMail(uint opUid, AdminCategory category, AddMailReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> DelMail(uint opUid, AdminCategory category, DelMailReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListServer(uint opUid, AdminCategory category, ListPageReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> SetResVerion(uint opUid, AdminCategory category, SetResVersionReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var grain = GrainFactory.GetGrain<IGlobalGrain>(0);
        ...
        private async Task<JsonResp> ReloadConfig(uint opUid, AdminCategory category)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListServer1(uint opUid, AdminCategory category)
        {
            // 要求至少代理权限
            if (category > AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var repo = DbService.Sql.GetRepository<ServerEntity>();
        ...
        private async Task<JsonResp> AddServer(uint opUid, AdminCategory category, AddServerReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var repo = DbService.Sql.GetRepository<ServerEntity>();
        ...
        private async Task<JsonResp> EditServer(uint opUid, AdminCategory category, EditServerReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ChangeServerStatus(uint opUid, AdminCategory category, ChangeServerStatusReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var entity = await DbService.Sql.Queryable<ServerEntity>().Where(it => it.Id == req.Id)
        ...
        private async Task<JsonResp> StartServer(uint opUid, AdminCategory category, uint serverId)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> StopServer(uint opUid, AdminCategory category, uint serverId)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var entity = await DbService.Sql.Queryable<ServerEntity>().Where(it => it.Id == serverId)
        ...
        private async Task<JsonResp> OpenActivity(uint opUid, AdminCategory category, OpenActivityReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> QueryCombineServer(uint opUid, AdminCategory category)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> CombineServer(uint opUid, AdminCategory category, CombineServerReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListUser(uint opUid, AdminCategory opCategory, ListUserReq req)
        {
            ...
            if (opCategory <= AdminCategory.Admin)
            {
                ...
                {
                    var entries = await RedisService.DandDeviceIdList();
        ...
        private async Task<JsonResp> EditUser(uint opUid, AdminCategory category, EditUserReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            ...
            // 代理只能修改自己名下的用户
            if (category >= AdminCategory.Agency && user.ParentId != opUid)
        ...
        private async Task<JsonResp> FrozeDevice(uint opUid, AdminCategory category, FrozeDeviceReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (string.IsNullOrEmpty(req.DeviceId) || !req.Status.HasValue)
                return JsonResp.Error(ErrCode.BadRequest);
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            ...
            {
                // 获取用户信息
                var userList = await DbService.Sql.Queryable<UserEntity>()
                    .Where(it => it.DeviceId == req.DeviceId)
                    .ToListAsync(it => it.Id);
                if (userList.Count <= 0) return JsonResp.Error("该设备码没有用户");
                // 添加封禁设备码
                _ = RedisService.AddBandDeviceId(req.DeviceId);

                foreach (var userId in userList)
                {
                    await RedisService.DelUserToken(userId);
                    var roles = await DbService.QueryRoles(userId);
            ...
            {
                // 删除封禁设备码
                _ = RedisService.DelBandDeviceId(req.DeviceId);
            }
            return JsonResp.Ok();
        }
        private async Task<JsonResp> FrozeUser(uint opUid, AdminCategory category, FrozeUserReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> GetRoleDetail(uint opUid, AdminCategory category, GetRoleDetailReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ChangeRoleStar(uint opUid, AdminCategory category, ChangeRoleStarReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> AddRoleSkillExp(uint opUid, AdminCategory category, GetRoleDetailReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> AddRoleEquip(uint opUid, AdminCategory category, AddRoleEquipReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> AddRoleOrnament(uint opUid, AdminCategory category, AddRoleOrnamentReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> AddRoleWing(uint opUid, AdminCategory category, AddRoleWingReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> AddRoleTitle(uint opUid, AdminCategory category, AddRoleTitleReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> DelRoleTitle(uint opUid, AdminCategory category, DelRoleTitleReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> DelRoleShane(uint opUid, AdminCategory category, DelRoleShaneReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> SetRoleType(uint opUid, AdminCategory category, SetRoleTypeReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> SetRoleFlag(uint opUid, AdminCategory category, SetRoleFlagReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
        ...
        private async Task<JsonResp> Recharge(uint opUid, AdminCategory category, RechargeReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (req.Id == 0 || req.Value == 0 || category >= AdminCategory.Agency)
                return JsonResp.BadRequest();
        ...
        private async Task<JsonResp> RechargeRole(uint opUid, AdminCategory opCategory, RechargeRoleReq req, bool bind = false)
        {
            ...
            // 充值记录
            {
                ...
                var rEntity = new RechargeRoleEntity
                {
                    ...
                    Money = costMoney,
                    Remark = (bind ? "积分单充" : "普通充值") + "|" + req.Remark,
                    CreateTime = TimeUtil.TimeStamp
                };
            ...
            try
            {
                ...
                if (bind)
                {
                    var bindPayRate = await RedisService.GetPayRateBindJade();
                    await grain.OnPayedBindJade(costMoney, costMoney * (int)bindPayRate);
                }
                else
                {
        ...
        /// 删除代理线下充值记录
        /// </summary>
        private async Task<JsonResp> DelRecharge(uint opUid, AdminCategory category, DelRecordsReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListRechargeRole(uint opUid, AdminCategory opCategory, ListRechargeRoleReq req)
        {
            ...
            else
            {
                ...
                // 代理只能查看自己下线用户
                var selector = DbService.Sql.Queryable<AdminEntity>()
                    .Where(it => it.Id == viewId)
                    .AsTreeCte()
                    .From<RechargeRoleEntity, RoleEntity>((_, b, c) => _)
                    .InnerJoin((a, b, c) => b.ParentId == a.Id)
                    .LeftJoin((a, b, c) => b.RoleId == c.Id)
        ...
        private async Task<JsonResp> DelRechargeRole(uint opUid, AdminCategory category, DelRecordsReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (req.StartTime > req.EndTime) return JsonResp.BadRequest();
        ...
        private async Task<JsonResp> DelPay(uint opUid, AdminCategory category, DelRecordsReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (req.StartTime > req.EndTime) return JsonResp.BadRequest();
        ...
        private async Task<JsonResp> ListPayRecords(uint opUid, AdminCategory opCategory, ListPayRecordsReq req)
        {
            ...
            var listAgency = await DbService.Sql.Queryable<AdminEntity>()
                .Where(it => it.ParentId == parentId && it.Category == AdminCategory.Agency && it.Status == AdminStatus.Normal)
            ...
            {
                // 先查找我下面所有的一级代理
                var sum = await DbService.Sql.Queryable<AdminEntity>()
        ...
        private async Task<JsonResp> ListRankLevel(uint opUid, AdminCategory category, ListRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> ListRankJade(uint opUid, AdminCategory category, ListRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> ListRankPay(uint opUid, AdminCategory category, ListRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> ListRankSldh(uint opUid, AdminCategory category, ListRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> ListRankSect(uint opUid, AdminCategory category, ListRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> DismissSect(uint opUid, AdminCategory category, DissmissSectReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ReloadSects(uint opUid, AdminCategory category, ReloadSectsReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListRankSinglePk(uint opUid, AdminCategory category, ListRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> ListOpLog(uint opUid, AdminCategory category, ListOpLogReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListBugLog(uint opUid, AdminCategory category, ListBugLogReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> DelBugLog(uint opUid, AdminCategory category, DelRecordsReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (req.StartTime > req.EndTime) return JsonResp.BadRequest();
        ...
        private async Task<JsonResp> SendPalaceNotice(uint opUid, AdminCategory category, uint serverId, string msg, uint times)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            var grain = GrainFactory.GetGrain<IServerGrain>(serverId);
        ...
        private async Task<JsonResp> SendRoleGift(uint opUid, AdminCategory category, uint roleId)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            // 获取角色信息
            var role = await DbService.Sql.Queryable<RoleEntity>()
                .Where(it => it.Id == roleId && (it.ParentId == opUid || category < AdminCategory.Agency))
                .FirstAsync();
            if (role == null) return JsonResp.Error("角色不存在");
        ...
        private async Task<JsonResp> SendSetLimitChargeRank(uint opUid, AdminCategory category, uint server, uint start, uint end, bool cleanup)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> SendGetLimitChargeRank(uint opUid, AdminCategory category, uint server)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
        ...
        private async Task<JsonResp> ListLimitRank(uint opUid, AdminCategory category, ListLimitRankReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> ListChatLog(uint opUid, AdminCategory category, ListChatLogReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            req.Build();
        ...
        private async Task<JsonResp> DelChatLog(uint opUid, AdminCategory category, DelRecordsReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (req.StartTime > req.EndTime) return JsonResp.BadRequest();
        ...
        readonly List<char> KEYLIST = new() { '2', '3', '4', '5', '6', '7', '8', '9',
         'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z' };
        private async Task<JsonResp> DelGenCDKey(uint opUid, AdminCategory category, GenCDKeyReq req)
        {
            // 要求管理员权限
            if (category >= AdminCategory.Agency) return JsonResp.Error(ErrCode.NoPermission);
            if (req.TypeId < (byte)CDKeyType.Charge ||
                req.TypeId > (byte)CDKeyType.ItemCharge ||
                req.Num <= 0 || req.Price <= 0) return JsonResp.BadRequest();
            var keyList = new List<string>();
            while (keyList.Count < req.Num)
            {
                for (int i = 0; i < req.Num; i++)
                {
                    ...
                    for (int j = 0; j < 6; j++)
                    {
                        key += KEYLIST[_random.Next(KEYLIST.Count)];
                    }
                    keyList.Add(key);
                }
                var existing = await DbService.Sql.Queryable<CDKeyEntity>().Where(it => keyList.Contains(it.Key)).ToListAsync();
                foreach (var e in existing)
                {
                    keyList.Remove(e.Key);
                }
            }
            var entityList = new List<CDKeyEntity>();
            var now = TimeUtil.TimeStamp;
            var extra = "";
            if (req.ItemId > 0 && req.ItemNum > 0)
            {
                extra = Json.SafeSerialize(new { item = req.ItemId, num = req.ItemNum });
            }
            ...
            var resultEntityList = await DbService.Sql.GetRepository<CDKeyEntity>().InsertAsync(entityList);
            var resultList = new List<string>();
            foreach (var e in resultEntityList)
            {
                if (e.Id > 0)
                {
        ...
        private void LogOp(string ip, string method, string args, uint uid = 0)
        {
            if (string.IsNullOrEmpty(args)) args = "{}";
            _logger.LogDebug($"{uid}|{ip}|{method}|{args}");
        }
        private void LogOp(string ip, string method, Immutable<byte[]> args, uint uid = 0)
        {
            LogOp(ip, method, args.Value == null ? null : System.Text.Encoding.UTF8.GetString(args.Value), uid);
        }
    }
}
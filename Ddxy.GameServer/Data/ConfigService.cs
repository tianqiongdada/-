...
namespace Ddxy.GameServer.Data
{
    public static class ConfigService
    {
        ...
        // 新手抽奖配置
        public static Draw1stConfig Draw1stConfig = new Draw1stConfig()
        {
            drawItems = new(),
            whiteList = new(),
        };
        ...
        public static Dictionary<uint, TianceConfig> TianceFuListAllBySkillId = new();
        // 天策符技能配置
        ...
        private static async Task ParseFile(string path, bool reload = false)
        {
            ...
            switch (shortName)
            {
                ...
                case "tiance":
                {
                    ...
                    TianceFuListAllBySkillId.Clear();
                    foreach (var (key, value) in dic)
                    {
                        // 全部
                        ...
                        TianceFuListAllBySkillId[(uint)value.skillId + 0] = value;
                        TianceFuListAllBySkillId[(uint)value.skillId + 1] = value;
                        TianceFuListAllBySkillId[(uint)value.skillId + 2] = value;
                    }
                    break;
                }
                // 天策符技能配置
                ...
                {
                    await using var stream = File.OpenRead(path);
                    var c = await JsonSerializer.DeserializeAsync<Draw1stConfig>(stream, Options);
                    Draw1stConfig.drawItems.Clear();
                    Draw1stConfig.drawItems.AddRange(c.drawItems);
                    Draw1stConfig.whiteList.Clear();
                    Draw1stConfig.whiteList.AddRange(c.whiteList);
                    break;
                }
                default:
                {

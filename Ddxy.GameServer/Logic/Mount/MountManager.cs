...
namespace Ddxy.GameServer.Logic.Mount
{
    public class MountManager
    {
        ...
        public async Task Unlock(uint id)
        {
            ...
            {
                var cfg = ConfigService.Items[itemId];
                _player.SendNotice($"<img src=\"{cfg.Icon}\"/><color=#28c873>[{cfg.Name}]不足");
                return;
            }
            ...
            {
                 var cfg = ConfigService.Items[itemId];
                _player.SendNotice($"<img src=\"{cfg.Icon}\"/><color=#28c873>[{cfg.Name}]</color>消耗失败");
                return;
            }
            mount.Locked = false;

...
namespace Ddxy.GameServer.Logic.Equip
{
    public class EquipManager
    {
        ...
        public async Task<Equip> AddEquip(EquipConfig cfg, bool notice = true)
        {
            ...
            _player.SendNotice($"恭喜您获得了{data.Cfg.Desc}<img src=\"{data.Cfg.Shape}\"/><color=#28c873>[{data.Cfg.Name}]</color>");
            // 神兵 仙器需要广播
            if (notice && data.Category >= EquipCategory.Shen)
            {
                var text =
                    $"<color=#00ff00>{_player.Entity.NickName}</c><color=#ffffff>获得了</c><color=#0fffff>{data.Cfg.Desc}<img src=\"{data.Cfg.Shape}\"/><color=#28c873>[{data.Cfg.Name}]</color></color>，<color=#ffffff>真是太幸运了</c>";
        ...
        public async ValueTask<bool> DelEquip(uint equipId, bool force = false, bool unInlay = true)
        {
            ...
            _player.LogDebug($"删除装备:{equip.Id} 配置ID:{equip.CfgId} 分类:{equip.Category} 品阶:{equip.Grade} 宝石:{equip.Gem} 位置:{equip.Cfg.Index}");
        ...
        public async Task<Ornament> AddOrnaments(OrnamentConfig cfg, int grade = 0)
        {
            ...
            _player.LogDebug($"添加配饰:{ornament.Id} 配置ID:{ornament.Cfg.Id} 品阶:{ornament.Grade}");
        ...
        public async Task DelOrnament(uint id, bool force = false)
        {
            ...
            _player.LogDebug($"删除配饰:{ornament.Id} 配置ID:{ornament.Cfg.Id} 品阶:{ornament.Grade}");
        }
        ...
        public async Task DecomposeOrnament(uint id)
        {
            ...
            _player.LogDebug($"分解配饰:{ornament.Id} 配置ID:{ornament.Cfg.Id} 品阶:{ornament.Grade}");
        }

        public async Task DecomposeOrnamentBatch(List<uint> idList)
        {
            var ornamentList = DelOrnamentBatch(idList, true);
            ...
            DelOrnamentBatch(filtered, false);
            // 获得玉符
            await _player.AddBagItem(100101, totalNum, tag: "分解佩饰");
        }

        private List<Ornament> DelOrnamentBatch(List<uint> list, bool test = false)
        {
            List<Ornament> ornamentList = new();
            ...
            foreach (var id in list)
            {
                ...
                // 只能丢弃背包里的配饰
                if (ornament.Place == EquipPlace.Bag)
                {
            ...
            // 从数据库中移除
            _ = DbService.DeleteEntity<OrnamentEntity>(ornamentIdList.ToArray());
            foreach (var ornament in ornamentList)
            {
                Ornaments.Remove(ornament.Id);
            }
            _ = _player.SendPacket(GameCmd.S2COrnamentDel, new S2C_OrnamentDel { List = { ornamentIdList } });
            return ornamentList;
        }
        ...
        {
            uint costBindJade = 10;
            var resp = new S2C_PetOrnamentDaZhao();
            if (_player.GetMoney(MoneyType.BindJade) < costBindJade)
            {
                resp.Error = $"积分不足";
            }
            ...
            {
                await _player.CostMoney(MoneyType.BindJade, costBindJade, false, "宠物配饰-打造");

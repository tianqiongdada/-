    ...
    public class Equip
    {
        ...
        public async Task XianQiUpgrade(uint use1, uint use2)
        {
            ...
            if (Cfg.Grade >= 5)
            {
                ...
                if (_player.GetBagItemNum(needItemId) < 1)
                {
                    ...
                    if (itemCfg != null)
                    {
                        _player.SendNotice($"缺少<img src=\"{itemCfg.Icon}\"/><color=#28c873>[{itemCfg.Name}]</color>");
                    }
                    return;
                }
            }
        ...
        public async Task DingZhi(List<int> attrs)
        {
            ...
            if (_player.GetBagItemNum(itemCfgId) < 1)
            {
                ...
                if (icfg != null)
                {
                    _player.SendNotice($"缺少<img src=\"{icfg.Icon}\"/><color=#28c873>[{icfg.Name}</color>");
                }
                else
                {
        ...
        public async Task GradeUpgrade()
        {
            ...
            {
                var itemCfg = ConfigService.Items[500052];
                _player.SendNotice($"缺少材料<img src=\"{itemCfg.Icon}\"/><color=#28c873>[{itemCfg.Name}]</color>");
                return;
            }

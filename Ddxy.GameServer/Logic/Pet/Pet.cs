...
namespace Ddxy.GameServer.Logic.Pet
{
    public class Pet
    {
        ...
        public Pet(PlayerGrain player, PetEntity entity, bool autoStart = true)
        {
            _player = player;
            Entity = entity;
            if (string.IsNullOrEmpty(Entity.Talks))
            {
                Entity.Talks = "[]";
            }
        ...
        public async ValueTask<bool> AddExp(ulong value, bool send = true)
        {
            ...
            if (send) _player.SendNotice($"获得 <color=#18ec10>{value}</color> <img src=\"9003\"/>宠物经验");
        ...
        public async Task SetTalks(string talks)
        {
            Entity.Talks = talks;
            await SendInfo();
        }

    ...
    public class XTaskManager
    {
        ...
        public bool IsTaskCompleted(uint taskId)
        {
            ...
            if (taskData != null)
            {
                ...
                {
                    return IsDailyTaskCompleted(taskData.Group);
                }
                else if (taskData.Kind == TaskKind.Instance)
                {
                    return IsInstanceTaskCompleted(taskId);
                }
            }
            return false;
        }
        ...
        public async Task InceptDailyTask(uint group, uint onlyId)
        {
            ...
            // 多人任务
            // 检查多人日常任务是否有队员已完成
            if (isTeamTask && GameDefine.CheckTaskTeamMemberHasCompleted)
            {
        ...
        public async Task InceptInstanceTask(uint taskId, uint onlyId)
        {
            ...
            // 检查多人副本任务是否有队员已完成
            if (GameDefine.CheckTaskTeamMemberHasCompleted)
            {
                    ...
                    foreach (var dnd in stepData.Npcs)
                    {
                        ...
                        if (GameDefine.CheckTaskTeamMemberHasCompleted)
                        {
                            ...
                            if (taskData != null)
                            {
                                ...
                                await _player.StartPve(npcOnlyId, npcCfg.MonsterGroup, BattleType.Normal, false, 0, (int)taskId);
                                found = true;
                                break;
                            }
                        }

    ...
    public class BattleGrain : Grain, IBattleGrain
    {
        ...
        private async Task DoRound()
        {
            ...
            foreach (var turn in _turns)
            {
                ...
                // 是否使用了加速技能？
#if false
                mb.speedRoleList = null;
                if (mb.ActionData.ActionType == BattleActionType.Skill && (mb.ActionData.ActionId == (uint)SkillId.QianKunJieSu || mb.ActionData.ActionId == (uint)SkillId.TianWaiFeiMo))
                {
                    ...
                    if (!mb.HasBuff(SkillType.Chaos))
                    {
                        ...
                        if (targetMb is {Dead: false})
                        {
                            if (targetMb.HasBuff(SkillType.YinShen))
                            {
                                ...
                                if (ret > 0 && (SkillManager.IsXianFa(skill.Type) || skill.Type == SkillType.GhostFire) && _random.Next(0, 100) < ret)
                                {
                    ...
                    {
                        _members.TryGetValue(targetList[trindex], out var trole);
                        if (trole == null) continue;

                        // 加速对负敏单位无效
                        if (trole.Spd < 0) break;
            ...
            for (var turnIndex = 0; turnIndex < _turns.Count; turnIndex++)
            {
                ...
                else if (mb.ActionData.ActionType == BattleActionType.Skill)
                {
                    ...
                    // 是否使用了加速技能？
#if false
                    if (mb.speedRoleList != null)
                    {
                        if (skillId == SkillId.QianKunJieSu || skillId == SkillId.TianWaiFeiMo)
                        {
                            targetList = mb.speedRoleList;
                        }
                        else
                        {
                            targetList.Clear();
                        }
                        mb.speedRoleList.Clear();
                    }
#endif
                    ...
                    for (var trindex = 0; trindex < targetList.Count; trindex++)
                    {
                        ...
                        if (hurt > 0)
                        {
                            ...
                            else
                            {
                                ...
                                if (skill.Type == SkillType.Frighten)
                                {
                                    var maxHurt = trole.Hp * effectData.HurtPercent / 100.0f;
                                    if (thurt > maxHurt)
                                    {
                                        thurt = maxHurt;
                                        orgHurt = thurt;
                                ...
                                // 切割
                                if (!_isPvp && mb.IsPlayer && mb.Data.QiegeLevel > 0 && thurt > 0) {
                                    var qhurt = trole.HpMax * ((ConfigService.QieGeLevelList[mb.Data.QiegeLevel].hp / 1000.0f) - (trole.Attrs.Get(AttrType.HdqieGe) / 100.0f));
                                    thurt += Math.Max(qhurt, 0f);
                                }
                                // 宠物 有觉醒技？
                                // 觉醒技 正气凛然

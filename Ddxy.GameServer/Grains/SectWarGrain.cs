...
namespace Ddxy.GameServer.Grains
{
    ...
    public class SectWarGrain : Grain, ISectWarGrain
    {
        ...
        // 开始比赛后，每10秒检查一次是否应该结束帮战
        private IDisposable _checkFinishTick;
        ...
        public Task ShutDown()
        {
            ...
            _checkFinishTick?.Dispose();
            _checkFinishTick = null;
        ...
        public async Task<Immutable<byte[]>> Enter(Immutable<byte[]> reqBytes)
        {
            ...
            // 已经入场，但是又调了一次入场，应该是那里出问题了，这里做个补救措施，直接下方成功
            if (_players.ContainsKey(req.Id))
            {
                _sects.TryGetValue(req.SectId, out var sectOld);
                _players.TryGetValue(req.Id, out var oldPlayer);
                if (sectOld != null && oldPlayer != null)
                {
                    resp.SectWarId = sectOld.SectWarId;
                    resp.Camp = sectOld.Camp;
                    resp.Place = oldPlayer.Place;
                    resp.State = oldPlayer.State;
                    // 强制重新同步位置和状态
                    oldPlayer.SyncAllForce(oldPlayer.Place, oldPlayer.State);
                    return new Immutable<byte[]>(Packet.Serialize(resp));
                }
                else
                {
        ...
        public async Task Online(uint roleId, uint teamLeader, uint sectId)
        {
            ...
            else if (_state == SectWarState.Fight && player != null)
            {
                ...
                if (sect != null)
                {
                    ...
                    // 强制重新同步位置和状态，可能出现在场外的玩家，强制拉进来
                    player.SyncAllForce(player.Place, player.State);
        ...
        private async Task OnActivityFight(object state)
        {
            ...
            foreach (var sid in _sects.Keys.ToList())
            {
                ...
                if (sect.Enemy == null || !sect.HasPlayer)
                {
                    // 算sect1轮空赢
                    ...
                    if (sect.Enemy != null)
                    {
                        RemoveSect(sect.Enemy);
                    }
                }
                else if (!sect.HasPlayer)
                {
            ...
            if (_sects.Count <= 1 || _players.Count <= 1)
            {
                // 结束
                LogDebug("全部轮空");
                await OnActivityFinish(null);
            ...
            else
            {
                ...
                _checkFinishTick?.Dispose();
                _checkFinishTick = null;
                _checkFinishTick = RegisterTimer(OnCheckActivityFinish, null, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(1));
            }
        }

        private Task OnCheckActivityFinish(object state)
        {
            _checkFinishTick?.Dispose();
            _checkFinishTick = null;
            if (!CheckFinish())
            {
                _checkFinishTick = RegisterTimer(OnActivityFinish, null, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(1));
            }
            return Task.CompletedTask;
        }

...
namespace Ddxy.GameServer.Grains
{
    ...
    public partial class PlayerGrain
    {
            ...
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    case GameCmd.C2SOrderItemShop:
                    {
                        ...
                        await ReqOrderItemShop(req.Id, (PayType)req.PayType);
                        break;
                    }
                    ...
                    case GameCmd.C2SOrnamentDecomposeBatch:
                    {
                        var req = C2S_OrnamentDecomposeBatch.Parser.ParseFrom(payload);
                        await EquipMgr.DecomposeOrnamentBatch(req.IdList.ToList());
                        break;
                    }
                    ...
                    // CD-KEY 兑换
                    case GameCmd.C2SCdkeyexchange:
                    {
                        var req = C2S_CDKEYExchange.Parser.ParseFrom(payload);
                        await ReqCDKeyExchange(req.Key);
                        break;
                    }
                    ...
                    case GameCmd.C2SPetSetTalk:
                    {
                        var req = C2S_PetSetTalk.Parser.ParseFrom(payload);
                        var pet = PetMgr.All.FirstOrDefault(p => p.Id == req.Id);
                        if (pet != null) await pet.SetTalks(req.Talks);
                        break;
                    }
                    // 觉醒突破
                    ...
                    */
                    ...
                    // 开始新手抽奖
                    case GameCmd.C2SMakeNewDraw:
                    {
                        //var req = C2S_MakeNewDraw.Parser.ParseFrom(payload);
                        await ReqMakeNewDraw();
                        break;
                    }
                    default:
        ...
        private async Task ReqEnterServer(C2S_EnterServer req)
        {
            ...
            // 新手抽奖
            Init1stDraw();
            if (_1stdrawItems.Count > 0)
            {
                var p = new S2C_EnterNewDraw();
                p.ItemList.AddRange(_1stdrawItems);
                await SendPacket(GameCmd.S2CEnterNewDraw, p);
            }
        }
        ...
        private async Task ReqCreateSect(string name, string desc)
        {
            ...
            // 检查充值额度
            if (!IsGm && Entity.TotalPayBS < 198)
            {
                SendNotice("充值额度不足198");
                return;
            }
        ...
        private async Task ReqContribSect(uint jade)
        {
            ...
            SendNotice($"帮派贡献+<img src=\"9000\"/><color=#18ec10>{jade}</color>");
        }
        ...
        public static bool CheckBanWord(string text)
        {
            ...
            if (numberList != null)
            {
                ...
                foreach (var c in text)
                {
                    ...
                    else if (numberList.Contains(c.ToString()))
                    {
                        // 超过XX个 封号
                        numCnt++;
                        if (numCnt >= ConfigService.BanChat.numberLimit)
            ...
            switch (req.Type)
            {
                ...
                case ChatMessageType.World:
                {
                    ...
                    if (Entity.Type != UserType.Gm)
                    {
                        ...
                        uint requiredPay = 30;
                        if ((nowT.Hour >= 0 && nowT.Hour < 9) || (nowT.Hour == 9 && nowT.Minute <= 59))
                        {
                            requiredPay = 100;
                        }
                        var x = Entity.TotalPayBS >= requiredPay;
                        if (!x)
                        {
                            SendNotice($"充值金额不足{requiredPay}元");
                            return;
                        }
                        var len = 0;
                        var start = false;
        ...
        {
            var resp = new S2C_EnterChargeUI() { JadeRate = await RedisService.GetPayRateJade(), BindJadeRate = await RedisService.GetPayRateBindJade() };
        ...
        // 物品商店--下单购买
        private async Task ReqOrderItemShop(uint id, PayType payType)
        {
            if (CheckSafeLocked()) return;
            ...
            if (string.IsNullOrWhiteSpace(_xinPayOptions.JadeNotifyUrl))
            {
                SendNotice("下单失败");
                return;
            }
            string payTypeStr = XinPayUtil.GetPayType(payType);
            if (string.IsNullOrWhiteSpace(payTypeStr))
            {
            ...
            uint amount = good.Price;
            string orderId = _xinPayOptions.OrderPrefix + entity.Id;
            string orderdatetime = DateTimeOffset.Now.ToString("yyyy-MM-dd HH:mm:ss");
            string url = XinPayUtil.Order(_xinPayOptions.MemberId, orderId, amount.ToString(), orderdatetime, payTypeStr,
        ...
        private async Task XinOrderJade(PayType payType, uint money)
        {
            if (string.IsNullOrWhiteSpace(_xinPayOptions.JadeNotifyUrl))
            {
                SendNotice("下单失败");
            ...
            money = 1;
            uint jade = money * await RedisService.GetPayRateJade();
            IBaseRepository<PayEntity> repository = DbService.Sql.GetRepository<PayEntity>();
            PayEntity entity = new PayEntity
            {
                Rid = RoleId,
            ...
            string orderdatetime = DateTimeOffset.Now.ToString("yyyy-MM-dd HH:mm:ss");
            string url = XinPayUtil.Order(_xinPayOptions.MemberId, orderId, amount.ToString(), orderdatetime, payTypeStr,
             _xinPayOptions.JadeNotifyUrl, _xinPayOptions.ReturnUrl, _xinPayOptions.SignMd5Key, RoleId);
        ...
        private async Task XinOrderBindJade(PayType payType, uint money)
        {
            ...
            uint bindjade = money * await RedisService.GetPayRateBindJade();
            IBaseRepository<PayEntity> repository = DbService.Sql.GetRepository<PayEntity>();
            PayEntity entity = new PayEntity
            {
                Rid = RoleId,
        ...
        {
            if (CheckSafeLocked()) return;
            if (cfgId != 50004 && cfgId != 50005) return;
        ...
        {
#if true
            if (GetMoney(MoneyType.BindJade) < 100)
            {
                SendNotice("积分不足，请先充积分");
                return;
            }
            var ret = await CostMoney(MoneyType.BindJade, 100, false, "重置累计充值奖励");
            if (!ret) return;
#else
            // 检查重置券是否足够？
            uint itemCfgId = 9905;
            if (GetBagItemNum(itemCfgId) <= 0)
            {
                var cfg = ConfigService.Items.GetValueOrDefault(itemCfgId, null);
                SendNotice(string.Format("{0}数量不足", cfg != null ? cfg.Name : "累计重置券"));
                return;
            }
            // 消耗重置券
            var ret = await AddBagItem(itemCfgId, -1, tag: "重置累计充值奖励");
            if (!ret) return;
#endif
            // 清空累计充值和累计充值领取记录
        ...
        private async Task ReqResetDailyPayRewards()
        {
            ...
            // 2w仙玉
            var ret = await CostMoney(MoneyType.Jade, 20000, false, "重置每日充值奖励");
            if (!ret) return;
        ...
        private async Task ReqShopItems(uint type)
        {
            ...
            {
                resp.List.Add(new ShopItem { CfgId = v.ItemId, Price = v.Price, CostType = costType, Discount = v.Discount });
            }
        ...
        private async Task ReqBianshenWuxingLevelUp(int type)
        {
            ...
            // 金木水火土满级为等级配置数量，五行最高等级为200
            if ((levels != null && next > levels.Count) || (next > 200 && levels == null))
            {
            ...
            // 升级所需材料ID
            uint itemCfgId = 9903;
            var levelCfg = levels.GetValueOrDefault(next, null);
            if (levelCfg == null)
            {
            ...
            if (type == 1)
            {
                var exp = (Bianshen.exp1 ?? 0) + itemCfg.Num;
                while (exp >= levelCfg.exp)
                {
                    // 升级
                    ...
                    current += 1;
                    next = current + 1;
                    levelCfg = levels.GetValueOrDefault(next, null);
                    if (levelCfg == null) break;
                }
                Bianshen.exp1 = exp;
            }
            else if (type == 2)
            {
                var exp = (Bianshen.exp2 ?? 0) + itemCfg.Num;
                while (exp >= levelCfg.exp)
                {
                    // 升级
                    Bianshen.wuxing[type] = next;
                    ...
                    current += 1;
                    next = current + 1;
                    levelCfg = levels.GetValueOrDefault(current, null);
                    if (levelCfg == null) break;
                }
                Bianshen.exp2 = exp;
            }
            ...
            {
                var exp = (Bianshen.exp3 ?? 0) + itemCfg.Num;
                while (exp >= levelCfg.exp)
                {
                    // 升级
                    Bianshen.wuxing[type] = next;
            ...
            {
                var exp = (Bianshen.exp4 ?? 0) + itemCfg.Num;
                while (exp >= levelCfg.exp)
                {
                    // 升级
                    Bianshen.wuxing[type] = next;
            ...
            {
                var exp = (Bianshen.exp5 ?? 0) + itemCfg.Num;
                while (exp >= levelCfg.exp)
                {
                    // 升级
                    Bianshen.wuxing[type] = next;
            ...
            await FreshAllSchemeAttrs();
            // 发送新的五行等级
            var resp = new S2C_BianshenWuxingLevelUp() { Type = type, Wuxing = JsonSerializer.Serialize(Bianshen) };
        ...
        private async Task ReqXingzhenDingZhi(int id, List<string> attrs)
        {
            ...
            var result = await AddBagItem(itemId, -itemNum, true, string.Format("定制{0}", config.name));
            if (!result)
            {
                SendNotice(string.Format("扣除<color=#ff0000>{0}</c>失败", ConfigService.Items[itemId].Name));
                return;
        ...
        private async Task ReqLuckyDrawInfo()
        {
            ...
            if (respBytes.Value != null)
            {
                ...
                await SendPacket(GameCmd.S2CLuckyDrawInfo, new S2C_LuckyDrawInfo()
                {
                    ...
                    CanOpenChest = !await RedisService.IsLuckyDrawChestGot(RoleId)
                });
            }
        }
        // 转盘--抽奖
        private async Task ReqLuckyDrawTurn(uint count)
        {
            ...
            if (cost > 0)
            {
                var ret = await CostMoney(MoneyType.BindJade, cost, tag: "转盘积分消耗");
                if (!ret)
                {
                    resp.ErrorMsg = "积分不足";
        ...
        private ExpExchangeInfo GetExpExchangeInfo()
        {
            ...
            return new ExpExchangeInfo()
            {
                ...
                // 积分消耗
                Cost = config == null ? uint.MaxValue : config.cost,
            };
        }
        // 经验兑换潜能--信息
        ...
        private async Task ReqExpExchange()
        {
            ...
            if ((Entity.Relive == info.Relive && Entity.Level >= info.Level) || Entity.Relive > info.Relive)
            {
                if (Entity.Exp >= info.NeedExp)
                {
                    if (GetMoney(MoneyType.BindJade) >= info.Cost)
                    {
                        if (await CostMoney(MoneyType.BindJade, info.Cost, tag: "经验兑换潜能消耗"))
                        {
                    ...
                    {
                        if (info.Cost == uint.MaxValue)
                        {
                            resp.Msg = "剩余兑换次数已经用完了";
                        }
                        else
                        {
                            resp.Msg = "当前积分不足，无法完成兑换";
                        }
                    }
                }
                else
        ...
        private bool CheckTianceFuCount(uint num = 1, bool notice = true)
        {
            var nowCount = TianceFuCount;
            if ((num + nowCount) > GameDefine.TianCeFuMaxCount)
            {
                SendNotice($"剩余位置不够{nowCount}/{GameDefine.TianCeFuMaxCount}，请分解足够空出位置");
                return false;
            }
            return true;
        }
        ...
        private async Task ReqTianceFuJianDing(TianceFuType type, uint num)
        {
            ...
            // 检查空位
            if (!CheckTianceFuCount(num)) return;
            // 检查类型对应的天策符
            ...
            // 消耗仙玉数量检查
            var needJade = num * GameDefine.TianCeFuJianDingCostJade;
        ...
        private async Task ReqTianceFuUse(string id, TianceFuState state)
        {
            ...
            // 检查空位
            if (state != TianceFuState.Unknown && !CheckTianceFuCount()) return;
            ConfigService.TianceFuListAllBySkillId.TryGetValue((uint)fu.SkillId, out var fconfig);
            if (fconfig != null)
            {
                if (fconfig.race.Count != 0 && !fconfig.race.Contains((Race)Entity.Race))
                {
                    SendNotice("种族不匹配");
                    return;
                }
        ...
        //切割--升级
        private async Task ReqQieGeUpgrade()
        {
            // 满级判断
            ...
            // 积分判断
            if (GetMoney(MoneyType.BindJade) <= 0)
            {
                SendNotice("积分不足");
                return;
            }
            ...
            var exp = QieGeExp + 100;
            while (exp >= levelConfig.exp)
            {
                exp -= levelConfig.exp;
                level += 1;
                levelConfig = ConfigService.QieGeLevelList.GetValueOrDefault(QieGeLevel, null);
                if (levelConfig == null)
                {
                    SendNotice("内部错误，请稍候再试");
                    return;
            ...
            await CostMoney(MoneyType.BindJade, 1, false, "切割升级--消耗");
            var needMapSync = false;
            if (level != QieGeLevel)
            {
                SendNotice($"恭喜神器切割升{level - QieGeLevel}级，到{level}级");
                needMapSync = true;
            }
            QieGeExp = exp;
            QieGeLevel = level;
        ...
        private async Task ReqRedSend(C2S_RedSend req)
        {
            ...
            if (req.Jade < 600000)
            {
                SendNotice("红包至少发<color=#18ec10>60W</color><img src=\"9004\"/>仙玉");
                return;
            }
            if (GetMoney(MoneyType.Jade) < req.Jade)
            {
                SendNotice("<img src=\"9004\"/>仙玉不足");
                return;
            }
        ...
        // 开始新手抽奖
        private async Task ReqMakeNewDraw()
        {
            if (_1stdrawItems.Count <= 0) return;
            var resp = new S2C_MakeNewDraw();

            // 扩充的物品索引列表
            var circleIndexList = new List<uint>();
            for (int j = 0; j < _1stdrawItemConfigs.Count; j++)
            {
                var ic = _1stdrawItemConfigs[j];
                var i = _1stdrawItems[j];
                circleIndexList.AddRange(Enumerable.Repeat(i.Index, (int)ic.rate).ToList());
            }
            resp.HitIndex = circleIndexList[Random.Next(0, circleIndexList.Count)];
            var hited = _1stdrawItems[(int)resp.HitIndex];
            // 白名单玩家直接返回，不发邮件
            if (ConfigService.Draw1stConfig.whiteList.Contains(RoleId))
            {
                Init1stDraw();
                resp.ItemList.AddRange(_1stdrawItems);
                await SendPacket(GameCmd.S2CMakeNewDraw, resp);
                return;
            }
            // 30天过期
            var now = DateTimeOffset.Now;
            var createTime = (uint)now.ToUnixTimeSeconds();
            var expireTime = createTime + 30 * 24 * 3600;
            // 分析items
            var dic = new Dictionary<uint, int>();
            dic[hited.CfgId] = (int)hited.Num;
            var items = Json.SafeSerialize(dic);
            // 发送邮件
            ...
            using var repo = DbService.Sql.GetRepository<MailEntity>();
            await repo.InsertAsync(entity);
            if (entity.Id > 0)
            {
                ExtEntity.Done1stDraw = true;
                Init1stDraw();
                _ = OnRecvMail(entity.Id);
                await SendPacket(GameCmd.S2CMakeNewDraw, resp);
            }
            else
            {
        ...
        private async Task ReqChallengeNpc(uint onlyId, uint cfgId)
        {
            ...
            else if (isEagle)
            {
                ...
                if (InTeam)
                {
                    if (IsTeamLeader)
                    {
                        var slist1 = await TeamGrain.CheckCanEagleKill();
                        if (slist1.Length > 0)
                        {
                            await SendNpcNotice(cfgId, $"{slist1}，转生等级不足3转，禁止挑战");
                            return;
                        }
                        var slist2 = await TeamGrain.CheckEagleKillCompleted();
                        if (slist2.Length > 0)
                        {
                            await SendNpcNotice(cfgId, $"{slist2}，今日次数已满");
                            return;
                        }
                    }
                    else
                    {
                        await SendNpcNotice(mod.CfgId, "只有队长可以挑战");
                        return;
                    }
                }
                if (Entity.Relive < 3)
                {
                    await SendNpcNotice(mod.CfgId, "转生等级不足3转，禁止挑战");
                    return;
                }
                // 每天只能打1次
        ...
        public async ValueTask<bool> AddBagItem(uint cfgId, int num, bool send = true, string tag = "")
        {
            ...
            else if (num < 0)
            {
                ...
                if (oldNum < val)
                {
                    SendNotice($"<img src=\"{itemCfg.Icon}\"/><color=#28c873>[{itemCfg.Name}]</color>数量不足");
                    return false;
                }
        ...
        public async ValueTask<long> AddMoney(byte type, long value, string tag = "", bool noNotice = false)
        {
        ...
        public async ValueTask<long> AddMoney(MoneyType type, long value, string tag = "", bool noNotice = false)
        {
            if (!IsActive) return 0;
            long res = value;
            switch (type)
        ...
        public async Task ExitBattle(Immutable<byte[]> reqBytes)
        {
            ...
            _campId = 0;
            if (IsEnterServer)
                await SendPacket(GameCmd.S2CBattleStop, new S2C_BattleStop { BattleId = req.Id, Win = isWin });
            // 观战 只退出观战
            if (_battleGrainWatched != null)
            {
            ...
            switch (req.Type)
            {
                ...
                default:
                {
                    ...
                    var reward = isWin;
                    // 已完成的任务，不给予任何奖励
                    if (!GameDefine.CheckTaskTeamMemberHasCompleted && req.TaskId >= 0 && TaskMgr.IsTaskCompleted((uint)req.TaskId))
                    {
                        reward = false;
                    }
                    // 任务相关
                    ...
                    if (reward && req.MonsterGroup > 0)
                    {
                        ...
                        if (groupCfg != null)
                        {
                            if (groupCfg.Exp > 0)
                            {
                                ...
                                if (expAddition > 0)
                                {
                                    SendNotice($"队长额外获得: <color=#18ec10>{expAddition * multiExp}</color><img src=\"9002\"/>角色经验");
                                }
                            }
                    ...
                    if (oldX2Exp > newX2Exp)
                    {
                        SendNotice($"已获双倍经验，还剩<img src=\"9220\"/>双倍经验点<color=#00ff00>{newX2Exp}</c>");
                    }
                }
                    break;
            }
        ...
        public async Task OnTeamJoinApplyAgree(uint teamId, uint teamLeader, byte teamTarget, uint teamSect)
        {
            if (!IsActive) return;
            // 申请被同意, 主动加入
            if (InTeam || InBattle || await IsSignedSinglePk())
            {
                return;
            }
        ...
        public async Task OnMallItemSelled(Immutable<byte[]> reqBytes)
        {
            ...
            // 提示
            SendNotice($"您摆摊的商品被人购买了，获得<color=#18ec10>{req.Reward}</color><img src=\"9001\"/>银币");
        }
        ...
        public async ValueTask<bool> OnStarBattle(uint npcOnlyId, byte level)
        {
            if (!IsActive) return false;
        ...
        public async Task OnHcPkResult(bool win)
        {
            ...
            SendNotice($"皇城PK您输掉了，扣<color=#18ec10>{subJade}</color><img src=\"9004\"/>仙玉！");
        }
        ...
        // 帮战强制同步位置、状态、阵营
        public async Task OnSectWarForceSync(byte place, byte state, int camp)
        {
            if (!IsActive) return;
            _sectWarCamp = camp;
            _sectWarState = (SectWarRoleState)state;
        ...
        public async ValueTask<int> OnPayed(int money, int jade, bool ignore = false)
        {
            if (!IsActive)
            {
                // 这里比较特殊，要确保是激活状态的
        ...
        public async ValueTask<int> OnPayedBindJade(int money, int bindJade)
        {
            if (!IsActive)
            {
                // 这里比较特殊，要确保是激活状态的
        ...
        public async ValueTask<string> GmSendRoleGift()
        {
            ...
            var changed = false;
#if false
           if (await AddBagItem(93001, (int)1, tag: "新手福利")){
               changed = true;
           }
           // if (await AddBagItem(93001, (int)1, tag: "新手福利")){
           //     changed = true;
        ...
        // 检查大鹏任务是否已完成
        public async ValueTask<bool> IsEagleKillCompleted()
        {
            await Task.CompletedTask;
            if (!IsActive) return true;
            return TaskMgr.EagleNum >= GameDefine.EagleNumDaily;
        }

        // 检查大鹏任务是否可以挑战
        public async ValueTask<bool> IsCanEagleKill()
        {
            await Task.CompletedTask;
            if (!IsActive) return false;
            return Entity.Relive >= 3;
        }
        ...
        // CD-KEY 兑换
        public async Task ReqCDKeyExchange(string key)
        {
            if (await RedisService.HasLockCDKey(key))
            {
                SendNotice("系统繁忙，请稍后再试！");
                return;
            }
            await RedisService.LockCDKey(key);
            var cdEntity = await DbService.Sql.Queryable<CDKeyEntity>()
                .Where(it => it.Key == key && it.DoneTime == 0).FirstAsync();
            if (cdEntity == null)
            {
                SendNotice("CD-KEY不存在，请确认输入是否正确！");
            }
            // 普通充值
            else if (cdEntity.TypeId == (byte)CDKeyType.Charge)
            {
                uint now = TimeUtil.TimeStamp;
                uint money = cdEntity.Price;
                uint jade = money * await RedisService.GetPayRateJade();
                using IRepositoryUnitOfWork uow = DbService.Sql.CreateUnitOfWork();
                // 创建订单
                var payEntity = await uow.GetRepository<PayEntity>().InsertAsync(new PayEntity
                {
                    Rid = RoleId,
                    Money = money,
                    Jade = jade,
                    BindJade = 0,
                    PayChannel = PayChannel.CDKEY,
                    PayType = PayType.CDKEY,
                    Remark = cdEntity.Extra,
                    Order = DateTime.Now.ToString("yyyyMMddHHmmssffff"),
                ...
                {
                    // 更新CD-KEY记录
                    if (await (from it in uow.Orm.Update<CDKeyEntity>()
                               where it.Id == cdEntity.Id && it.DoneTime == 0
                               select it)
                       .Set((CDKeyEntity it) => it.DoneTime, now)
                    ...
                    {
                        // 更新充值记录
                        if (await OnPayed((int)payEntity.Money, (int)payEntity.Jade) == 0)
                        {
                            uow.Rollback();
                        }
                        else
            ...
            // 积分充值
            else if (cdEntity.TypeId == (byte)CDKeyType.BindJadeCharge)
            {
                uint now = TimeUtil.TimeStamp;
                uint money = cdEntity.Price;
                uint bindjade = money * await RedisService.GetPayRateBindJade();
                ...
                else
                {
                    ...
                    {
                        // 更新充值记录
                        if (await OnPayedBindJade((int)payEntity.Money, (int)payEntity.BindJade) == 0)
                        {
                            uow.Rollback();
                        }
                        else
            ...
            // 物品充值
            else if (cdEntity.TypeId == (byte)CDKeyType.ItemCharge)
            {
                uint now = TimeUtil.TimeStamp;
                uint money = cdEntity.Price;
                using IRepositoryUnitOfWork uow = DbService.Sql.CreateUnitOfWork();
                ...
                else
                {
                    ...
                    {
                        var extraInfo = Json.SafeDeserialize<CDKEYExtra>(cdEntity.Extra);
                        if (extraInfo == null || extraInfo.item <= 0 || extraInfo.num <= 0)
                        {
                            uow.Rollback();
                            SendNotice("下单失败, 请再次尝试");
                        }
                        // 更新充值记录
                        else if (await OnPayedItem(extraInfo.item, extraInfo.num, (int)cdEntity.Price) == 0)
                        {
                            uow.Rollback();
                            SendNotice("下单失败, 请再次尝试");
                        }
            ...
            {
                SendNotice("系统错误，请稍后再试！");
            }
            await RedisService.UnLockCDKey(key);
        }

...
namespace Ddxy.GameServer.Grains
{
    ...
    public class TeamGrain : Grain, ITeamGrain
    {
        ...
        // 检查大鹏任务是否已完成
        public async ValueTask<string> CheckEagleKillCompleted()
        {
            if (!_isActive) return "尚未激活";
            string slist = "";
            foreach (var tod in _queue)
            {
                ...
                if (grain != null)
                {
                    if (await grain.IsEagleKillCompleted())
                    {
                        slist += slist.Length == 0 ? tod.Name : "，" + tod.Name;
                    }
                }
        ...
        // 检查大鹏任务是否可以挑战
        public async ValueTask<string> CheckCanEagleKill()
        {
            if (!_isActive) return "尚未激活";
            string slist = "";
            foreach (var tod in _queue)

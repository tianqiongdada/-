...
namespace Ddxy.GameServer.Grains
{
    ...
    public class BianshenObject
    {
        ...
        // 五行修炼经验，金、木、水、火、土、五行，对应1、2、3、4、5、6
    ...
    public partial class PlayerGrain : Grain, IPlayerGrain, IGrainWithIntegerKey, IGrain, IAddressable
    {
        ...
        public int TianceFuCount => Tiance.list.Count(f => f.State == TianceFuState.Unknown);
        ...
        // 新手抽奖奖项
        private List<LuckyDrawItem> _1stdrawItems = new();
        private List<DrawItem> _1stdrawItemConfigs = new();
        ...
        public async ValueTask<bool> StartPve(uint source, uint group = 10048, BattleType type = BattleType.Normal,
            bool yewai = false, byte starLevel = 0, int taskId = -1)
        {
            if (!IsActive) return false;
        ...
        public async Task UseItem(ItemConfig cfg, uint num, uint target)
        {
            ...
            switch (cfg.Id)
            {
                ...
                case 80001:
                {
                    ...
                    await AddBagItem(cfg.Id, -(int)num, tag: "UseItem");
                    long current = GetMoney(MoneyType.Silver);
                    long addon = num * (long)cfg.Num;
                    long after = current + addon;
                    uint max = 4000000000;
                    if (after > max) {
                        await AddMoney(MoneyType.Silver, max - current, "使用一箱银币");
                    } else {
                        await AddMoney(MoneyType.Silver, addon, "使用一箱银币");
                    }
                    break;
                }
                case 10101:
                ...
                case 9918:
                {
                    ...
                    SendNotice($"<color=#18ec10>+{cfg.Num}</color><img src=\"9220\"/>双倍经验点");
                }
                break;
                // 飞升券
                case 100327:
                ...
                case 500046:
                {
                    ...
                    var id = l[Random.Next(l.Count)];
                    await AddBagItem(id, 1, tag: "物品使用");
                    var itemCfg = ConfigService.Items[id];
                    SendNotice($"恭喜获得一个<img src=\"{itemCfg.Icon}\"/><color=#28c873>[{itemCfg.Name}]</color>");
                }
                break;
                // 全服烟花
                case 500057:
                ...
                case 500051:
                {
                    Dictionary<uint, int> result = new() { [500047] = 0, [500048] = 0, [500049] = 0 };
                    int used = 0;
                    int needSlot = 0;
                    for (int i = 0; i < num; i++)
                    {
                        if (GetBagItemNum(cfg.Id) < 0) break;
                        List<uint> l = new() { 500047, 500048, 500049, };
                        var id = l[Random.Next(l.Count)];
                        // 不占格子
                        if (Items.ContainsKey(id) || result[id] > 0)
                        {
                            used++;
                            result[id]++;
                            continue;
                        ...
                        // 格子剩余足够
                        if ((GameDefine.LimitBagItemKindNum - BagCount) > needSlot)
                        {
                            used++;
                            result[id]++;
                            needSlot++;
                    ...
                    foreach (var (id, c) in result)
                    {
                        // SendNotice($"恭喜获得{c}个{ConfigService.Items[id].Name}");
                        await AddBagItem(id, c, tag: "物品使用");
                    }
                    await AddBagItem(cfg.Id, -used, tag: "UseItem");
                }
                break;
                // 炼化锁--直接给1积分
                case 9902:
                {
                    if (GetBagItemNum(cfg.Id) < num) return;
                    await AddBagItem(cfg.Id, -(int)num, tag: "UseItem");
                    await AddMoney(MoneyType.BindJade, num, tag: "炼化锁");
                }
                break;
                // 充值券
                case 500058:
                ...
                case 500063:
                {
                    ...
                    var yuan = cfg.Id == 500058 ? Random.Next(cfg.Num) + 1 : cfg.Num;
                    await OnPayed(yuan, yuan * (int)payRate);
                }
                break;
                default:
                {
        ...
        private void InitTiance()
        {
            ...
            foreach (var fu in Tiance.list)
            {
                ...
                if (fconfig != null)
                {
                    ...
                    {
                        fu.State = TianceFuState.Unknown;
                        changed = true;
                    }
                }
            }
        ...
        // 初始新手抽奖
        private void Init1stDraw()
        {
            _1stdrawItems.Clear();
            _1stdrawItemConfigs.Clear();
            if (!ExtEntity.Done1stDraw || ConfigService.Draw1stConfig.whiteList.Contains(RoleId))
            {
                var itemList = ConfigService.Draw1stConfig.drawItems;
                while (_1stdrawItems.Count < 8)
                {
                    var ri = itemList[Random.Next(itemList.Count)];
                    _1stdrawItems.Add(new LuckyDrawItem() { CfgId = ri.id, Num = ri.num, Index = (uint)_1stdrawItems.Count });
                    _1stdrawItemConfigs.Add(ri);
                }
            }
        }

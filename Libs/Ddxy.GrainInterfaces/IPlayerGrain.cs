...
namespace Ddxy.GrainInterfaces
{
    public interface IPlayerGrain : IGrainWithIntegerKey
    {
        ...
        /// 添加Money
        /// </summary>
        ValueTask<long> AddMoney(byte type, long value, string tag = "", bool noNotice = false);
        ...
        Task OnSectWarForceSync(byte place, byte state, int camp);
        ...
        ValueTask<int> OnPayed(int money, int jade, bool ignore = false);

        ValueTask<int> OnPayedBindJade(int money, int bindJade);
        ...
        // 检查大鹏任务是否已完成
        ValueTask<bool> IsEagleKillCompleted();

        // 检查大鹏任务是否可以挑战
        ValueTask<bool> IsCanEagleKill();
